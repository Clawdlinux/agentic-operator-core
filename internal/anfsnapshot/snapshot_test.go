...
func TestCaptureMeasuresExactOutputAndReportsReduction(t *testing.T) {
	...
	if result.Metrics.RawBytes != len(result.RawJSON) || result.Metrics.ANFBytes != len(result.ANF) {
		t.Fatalf("byte measurements = %#v, raw=%d anf=%d", result.Metrics, len(result.RawJSON), len(result.ANF))
	}
	if result.Metrics.RawTokensEst != estimateTokens(len(result.RawJSON)) || result.Metrics.ANFTokensEst != estimateTokens(len(result.ANF)) {
		t.Fatalf("token estimates do not use chars/4: %#v", result.Metrics)
	}
	wantReduction := float64(len(result.RawJSON)-len(result.ANF)) / float64(len(result.RawJSON)) * 100
